...
with open(csv_file, mode='r') as file:
    ...
    pre_hook= [
        ...
        "ALTER TABLE bronze_zone_dev.{schema}.{snapshot_table_name} ADD COLUMN HASH_KEY STRING",
        "{{{{ copy_into_macro('bronze_zone_dev.{schema}.{schema}_stage', 'bronze_zone_dev.{schema}.{transient_table_name}', true, '{file_location}') }}}}",
        "UPDATE bronze_zone_dev.{schema}.{transient_table_name} SET AUDIT_CREATED_DATETIME = CURRENT_TIMESTAMP(), AUDIT_CREATED_BY = CURRENT_USER() WHERE AUDIT_INGEST_DATETIME > (SELECT COALESCE(MAX(AUDIT_CREATED_DATETIME), TO_TIMESTAMP('1900-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')) FROM bronze_zone_dev.{schema}.{transient_table_name})"
    ],
    post_hook= [
