{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.boat_add_int_rel_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.boat_add_int_rel",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.boat_add_int_rel AS SELECT * FROM bronze_zone_dev.wescom_pl.boat_add_int_rel_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.boat_add_int_rel_transient', true, '(?i)TDB2WPLQ.BOAT_ADD_INT_REL/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.boat_add_int_rel AS SELECT * FROM bronze_zone_dev.wescom_pl.boat_add_int_rel_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.BOAT_ID) OR TO_CHAR(t.ADD_INT_ID) IS NULL THEN 'I'
