{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.undly_unit_cov_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.undly_unit_cov",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.undly_unit_cov AS SELECT * FROM bronze_zone_dev.wescom_pl.undly_unit_cov_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.undly_unit_cov_transient', true, '(?i)TDB2WPLQ.UNDLY_UNIT_COV/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.undly_unit_cov AS SELECT * FROM bronze_zone_dev.wescom_pl.undly_unit_cov_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.UNDLY_UNIT_COV_ID) IS NULL THEN 'I'
