{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.prch_prdt_info_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.prch_prdt_info",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.prch_prdt_info AS SELECT * FROM bronze_zone_dev.wescom_pl.prch_prdt_info_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.prch_prdt_info_transient', true, '(?i)TDB2WPLQ.PRCH_PRDT_INFO/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.prch_prdt_info AS SELECT * FROM bronze_zone_dev.wescom_pl.prch_prdt_info_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.VEND_PRDT_NM) OR TO_CHAR(t.PRDT_CTGZ_CD) IS NULL THEN 'I'
