{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.insd_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.insd",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.insd AS SELECT * FROM bronze_zone_dev.wescom_pl.insd_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.insd_transient', true, '(?i)TDB2WPLQ.INSD/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.insd AS SELECT * FROM bronze_zone_dev.wescom_pl.insd_transient"
    ]
) }}
