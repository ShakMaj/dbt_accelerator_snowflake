{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.mc_mnfctr_dom_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.mc_mnfctr_dom",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.mc_mnfctr_dom AS SELECT * FROM bronze_zone_dev.wescom_pl.mc_mnfctr_dom_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.mc_mnfctr_dom_transient', true, '(?i)TDB2WPLQ.MC_MNFCTR_DOM/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.mc_mnfctr_dom AS SELECT * FROM bronze_zone_dev.wescom_pl.mc_mnfctr_dom_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.MC_MNFCTR_CD) IS NULL THEN 'I'
