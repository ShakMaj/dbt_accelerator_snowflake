{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.mc_assc_ver_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.mc_assc_ver",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.mc_assc_ver AS SELECT * FROM bronze_zone_dev.wescom_pl.mc_assc_ver_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.mc_assc_ver_transient', true, '(?i)TDB2WPLQ.MC_ASSC_VER/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.mc_assc_ver AS SELECT * FROM bronze_zone_dev.wescom_pl.mc_assc_ver_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.VER_ID) OR TO_CHAR(t.MC_ASSC_ID) IS NULL THEN 'I'
