{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.two_term_endr_sts_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.two_term_endr_sts",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.two_term_endr_sts AS SELECT * FROM bronze_zone_dev.wescom_pl.two_term_endr_sts_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.two_term_endr_sts_transient', true, '(?i)TDB2WPLQ.TWO_TERM_ENDR_STS/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.two_term_endr_sts AS SELECT * FROM bronze_zone_dev.wescom_pl.two_term_endr_sts_transient"
    ]
) }}
