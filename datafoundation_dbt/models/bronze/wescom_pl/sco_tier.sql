{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.sco_tier_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.sco_tier",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.sco_tier AS SELECT * FROM bronze_zone_dev.wescom_pl.sco_tier_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.sco_tier_transient', true, '(?i)TDB2WPLQ.SCO_TIER/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.sco_tier AS SELECT * FROM bronze_zone_dev.wescom_pl.sco_tier_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.SCO_TIER_VER_ID) OR TO_CHAR(t.SCO_CD) OR TO_CHAR(t.SCO_MDL_CD) IS NULL THEN 'I'
