{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.rel_pol_func_typ_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.rel_pol_func_typ",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.rel_pol_func_typ AS SELECT * FROM bronze_zone_dev.wescom_pl.rel_pol_func_typ_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.rel_pol_func_typ_transient', true, '(?i)TDB2WPLQ.REL_POL_FUNC_TYP/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.rel_pol_func_typ AS SELECT * FROM bronze_zone_dev.wescom_pl.rel_pol_func_typ_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.REL_POL_FUNC_TYP) IS NULL THEN 'I'
