{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.display_type_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.display_type",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.display_type AS SELECT * FROM bronze_zone_dev.wescom_pl.display_type_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.display_type_transient', true, '(?i)TDB2WPLQ.DISPLAY_TYPE/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.display_type AS SELECT * FROM bronze_zone_dev.wescom_pl.display_type_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.DSPL_TYP) IS NULL THEN 'I'
