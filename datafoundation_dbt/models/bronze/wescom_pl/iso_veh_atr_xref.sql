{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.iso_veh_atr_xref_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.iso_veh_atr_xref",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.iso_veh_atr_xref AS SELECT * FROM bronze_zone_dev.wescom_pl.iso_veh_atr_xref_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.iso_veh_atr_xref_transient', true, '(?i)TDB2WPLQ.ISO_VEH_ATR_XREF/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.iso_veh_atr_xref AS SELECT * FROM bronze_zone_dev.wescom_pl.iso_veh_atr_xref_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.VER_ID) OR TO_CHAR(t.ISO_VEH_ATR_CD) OR TO_CHAR(t.FTUR_TYP_CD) OR TO_CHAR(t.MDL_YR) IS NULL THEN 'I'
