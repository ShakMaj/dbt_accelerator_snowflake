{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.rel_pol_func_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.rel_pol_func",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.rel_pol_func AS SELECT * FROM bronze_zone_dev.wescom_pl.rel_pol_func_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.rel_pol_func_transient', true, '(?i)TDB2WPLQ.REL_POL_FUNC/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.rel_pol_func AS SELECT * FROM bronze_zone_dev.wescom_pl.rel_pol_func_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.VER_ID) OR TO_CHAR(t.REL_WFLD_POL_SYM) OR TO_CHAR(t.REL_POL_FUNC_TYP) OR TO_CHAR(t.REL_SEQ_NR) IS NULL THEN 'I'
