{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.stmt_atr_list_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.stmt_atr_list",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.stmt_atr_list AS SELECT * FROM bronze_zone_dev.wescom_pl.stmt_atr_list_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.stmt_atr_list_transient', true, '(?i)TDB2WPLQ.STMT_ATR_LIST/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.stmt_atr_list AS SELECT * FROM bronze_zone_dev.wescom_pl.stmt_atr_list_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.VER_ID) OR TO_CHAR(t.STMT_GRP_ID) OR TO_CHAR(t.STMT_CD) OR TO_CHAR(t.DSPL_SEQ_NR) OR TO_CHAR(t.STMT_ATR_TYP_CD) IS NULL THEN 'I'
