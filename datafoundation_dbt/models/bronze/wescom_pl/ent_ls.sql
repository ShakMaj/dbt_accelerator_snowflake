{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.ent_ls_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.ent_ls",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.ent_ls AS SELECT * FROM bronze_zone_dev.wescom_pl.ent_ls_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.ent_ls_transient', true, '(?i)TDB2WPLQ.ENT_LS/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.ent_ls AS SELECT * FROM bronze_zone_dev.wescom_pl.ent_ls_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.ENT_LS_ID) IS NULL THEN 'I'
