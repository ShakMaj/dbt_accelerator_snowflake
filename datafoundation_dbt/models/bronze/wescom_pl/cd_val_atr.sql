{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.cd_val_atr_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.cd_val_atr",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.cd_val_atr AS SELECT * FROM bronze_zone_dev.wescom_pl.cd_val_atr_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.cd_val_atr_transient', true, '(?i)TDB2WPLQ.CD_VAL_ATR/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.cd_val_atr AS SELECT * FROM bronze_zone_dev.wescom_pl.cd_val_atr_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.VER_ID) OR TO_CHAR(t.CD_TYP_CD) OR TO_CHAR(t.CD_VAL_CD) OR TO_CHAR(t.CD_VAL_ATR_CODE) IS NULL THEN 'I'
