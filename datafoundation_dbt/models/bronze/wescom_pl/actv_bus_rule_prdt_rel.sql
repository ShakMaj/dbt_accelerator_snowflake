{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_pl.actv_bus_rule_prdt_rel_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_pl.actv_bus_rule_prdt_rel",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_pl.actv_bus_rule_prdt_rel AS SELECT * FROM bronze_zone_dev.wescom_pl.actv_bus_rule_prdt_rel_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.wescom_pl.wescom_pl_stage', 'bronze_zone_dev.wescom_pl.actv_bus_rule_prdt_rel_transient', true, '(?i)TDB2WPLQ.ACTV_BUS_RULE_PRDT_REL/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_pl.actv_bus_rule_prdt_rel AS SELECT * FROM bronze_zone_dev.wescom_pl.actv_bus_rule_prdt_rel_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.MSG_CD) OR TO_CHAR(t.SEQ_NR) OR TO_CHAR(t.PRDT_CD) OR TO_CHAR(t.XCLD_IND) IS NULL THEN 'I'
