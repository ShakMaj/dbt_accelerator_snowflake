{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.sasreference.reqpolicy_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.sasreference.reqpolicy",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.sasreference.reqpolicy AS SELECT * FROM bronze_zone_dev.sasreference.reqpolicy_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.sasreference.sasreference_stage', 'bronze_zone_dev.sasreference.reqpolicy_transient', true, '(?i)DBO.ReqPolicy/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_sasreference.reqpolicy AS SELECT * FROM bronze_zone_dev.sasreference.reqpolicy_transient"
    ]
) }}
