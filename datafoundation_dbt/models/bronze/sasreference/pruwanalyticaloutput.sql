{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.sasreference.pruwanalyticaloutput_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.sasreference.pruwanalyticaloutput",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.sasreference.pruwanalyticaloutput AS SELECT * FROM bronze_zone_dev.sasreference.pruwanalyticaloutput_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.sasreference.sasreference_stage', 'bronze_zone_dev.sasreference.pruwanalyticaloutput_transient', true, '(?i)DBO.PruwAnalyticalOutput/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_sasreference.pruwanalyticaloutput AS SELECT * FROM bronze_zone_dev.sasreference.pruwanalyticaloutput_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.ID) OR TO_CHAR(t.REQUESTID) OR TO_CHAR(t.PGID) OR TO_CHAR(t.LINEOFBUSINESS) OR TO_CHAR(t.VARIABLE) OR TO_CHAR(t.VALUETYPE) IS NULL THEN 'I'
