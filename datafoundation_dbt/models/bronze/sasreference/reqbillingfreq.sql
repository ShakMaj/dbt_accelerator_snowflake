{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.sasreference.reqbillingfreq_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.sasreference.reqbillingfreq",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.sasreference.reqbillingfreq AS SELECT * FROM bronze_zone_dev.sasreference.reqbillingfreq_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.sasreference.sasreference_stage', 'bronze_zone_dev.sasreference.reqbillingfreq_transient', true, '(?i)DBO.ReqBillingFreq/.*.parquet') }}",
         "UPDATE bronze_zone_dev.sasreference.reqbillingfreq_transient SET AUDIT_CREATED_DATETIME = CURRENT_TIMESTAMP(), AUDIT_CREATED_BY = CURRENT_USER(), HASH_KEY_COLUMNS = SHA2(CONCAT(ID,' | ',REQUESTID,' | ',PGID,' | ',POLICYNUM,' | ',POLICYSOURCETYPE)) WHERE HASH_KEY_COLUMNS IS NULL"
    ],
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_sasreference.reqbillingfreq AS SELECT * FROM bronze_zone_dev.sasreference.reqbillingfreq_transient"
    ]
) }}

-- Step 1: Source Data with FLAG Calculation
WITH reqbillingfreq_source_data AS (
    SELECT 
        -- Dynamically get all columns from the source table except 'CREATED_BY' and 'CREATED_DATETIME'
        {{ get_dynamic_columns('SASREFERENCE', 'REQBILLINGFREQ_TRANSIENT') }},
    FROM 
        bronze_zone_dev.sasreference.reqbillingfreq_transient s
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.ID) OR TO_CHAR(t.REQUESTID) OR TO_CHAR(t.PGID) OR TO_CHAR(t.POLICYNUM) OR TO_CHAR(t.POLICYSOURCETYPE) IS NULL THEN 'I'
            ELSE 'U'  -- Update if target ID exists
        END AS UPSERT_FLAG
    FROM reqbillingfreq_source_data s
    LEFT JOIN {{ this }} t
