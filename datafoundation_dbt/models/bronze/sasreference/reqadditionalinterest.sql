{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.sasreference.reqadditionalinterest_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.sasreference.reqadditionalinterest",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.sasreference.reqadditionalinterest AS SELECT * FROM bronze_zone_dev.sasreference.reqadditionalinterest_transient WHERE 1 = 0",
        "{{ copy_into_macro('bronze_zone_dev.sasreference.sasreference_stage', 'bronze_zone_dev.sasreference.reqadditionalinterest_transient', true, '(?i)DBO.ReqAdditionalInterest/.*.parquet') }}",
    ...
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_sasreference.reqadditionalinterest AS SELECT * FROM bronze_zone_dev.sasreference.reqadditionalinterest_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.ID) OR TO_CHAR(t.ID) OR TO_CHAR(t.REQUESTID) OR TO_CHAR(t.PGID) OR TO_CHAR(t.POLICYNUM) OR TO_CHAR(t.LINEOFBUSINESS) OR TO_CHAR(t.AITYPE) OR TO_CHAR(t.POLICYSOURCETYPE) IS NULL THEN 'I'
