{{ config(
    ...
    pre_hook= [
        "{{ copy_into_macro('bronze_zone_dev.wescom_policy_mgt_ext.wescom_policy_mgt_ext_stage', 'bronze_zone_dev.wescom_policy_mgt_ext.get_spss_data_rspn_xml_transient', false, '(?i)TDB2PMED.GET_SPSS_DATA_RSPN_XML/.*.parquet') }}",
         "UPDATE bronze_zone_dev.wescom_policy_mgt_ext.get_spss_data_rspn_xml_transient SET AUDIT_CREATED_DATETIME = CURRENT_TIMESTAMP(), AUDIT_CREATED_BY = CURRENT_USER(), HASH_KEY_COLUMNS = SHA2(CONCAT(GET_SPSS_DATA_RSPN_XML_ID)) WHERE HASH_KEY_COLUMNS IS NULL"
    ]
) }}
