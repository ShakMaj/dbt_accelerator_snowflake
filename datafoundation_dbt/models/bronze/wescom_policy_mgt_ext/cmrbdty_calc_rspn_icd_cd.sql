{{ config(
    ...
    pre_hook= [
        "{{ copy_into_macro('bronze_zone_dev.wescom_policy_mgt_ext.wescom_policy_mgt_ext_stage', 'bronze_zone_dev.wescom_policy_mgt_ext.cmrbdty_calc_rspn_icd_cd_transient', false, '(?i)TDB2PMED.CMRBDTY_CALC_RSPN_ICD_CD/.*.parquet') }}",
         "UPDATE bronze_zone_dev.wescom_policy_mgt_ext.cmrbdty_calc_rspn_icd_cd_transient SET AUDIT_CREATED_DATETIME = CURRENT_TIMESTAMP(), AUDIT_CREATED_BY = CURRENT_USER(), HASH_KEY_COLUMNS = SHA2(CONCAT(CMRBDTY_CALC_RSPN_ICD_CD_ID)) WHERE HASH_KEY_COLUMNS IS NULL"
    ]
) }}
