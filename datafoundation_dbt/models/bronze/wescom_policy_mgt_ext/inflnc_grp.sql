{{ config(
    ...
    pre_hook= [
        "{{ copy_into_macro('bronze_zone_dev.wescom_policy_mgt_ext.wescom_policy_mgt_ext_stage', 'bronze_zone_dev.wescom_policy_mgt_ext.inflnc_grp_transient', false, '(?i)TDB2PMED.INFLNC_GRP/.*.parquet') }}",
         "UPDATE bronze_zone_dev.wescom_policy_mgt_ext.inflnc_grp_transient SET AUDIT_CREATED_DATETIME = CURRENT_TIMESTAMP(), AUDIT_CREATED_BY = CURRENT_USER(), HASH_KEY_COLUMNS = SHA2(CONCAT(INFLNC_GRP_ID)) WHERE HASH_KEY_COLUMNS IS NULL"
    ]
) }}
