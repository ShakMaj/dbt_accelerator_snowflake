{{ config(
    ...
    pre_hook= [
        "delete from bronze_zone_dev.wescom_policy_mgt_ext.rfrrl_transient",
        "DROP TABLE IF EXISTS bronze_zone_dev.wescom_policy_mgt_ext.rfrrl",
        "CREATE TABLE IF NOT EXISTS bronze_zone_dev.wescom_policy_mgt_ext.rfrrl AS SELECT * FROM bronze_zone_dev.wescom_policy_mgt_ext.rfrrl_transient WHERE 1 = 0",
        "ALTER TABLE bronze_zone_dev.wescom_policy_mgt_ext.rfrrl ADD COLUMN HASH_KEY STRING",
        "{{ copy_into_macro('bronze_zone_dev.wescom_policy_mgt_ext.wescom_policy_mgt_ext_stage', 'bronze_zone_dev.wescom_policy_mgt_ext.rfrrl_transient', true, '.*WescomParquet/TDB2PMED.RFRRL/.*.parquet') }}",
        "UPDATE bronze_zone_dev.wescom_policy_mgt_ext.rfrrl_transient SET AUDIT_CREATED_DATETIME = CURRENT_TIMESTAMP(), AUDIT_CREATED_BY = CURRENT_USER() WHERE AUDIT_INGEST_DATETIME > (SELECT COALESCE(MAX(AUDIT_CREATED_DATETIME), TO_TIMESTAMP('1900-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')) FROM bronze_zone_dev.wescom_policy_mgt_ext.rfrrl_transient)"
    ],
    post_hook= [
        "CREATE OR REPLACE VIEW publish_zone_dev.BRONZE_wescom_policy_mgt_ext.rfrrl AS SELECT * FROM bronze_zone_dev.wescom_policy_mgt_ext.rfrrl_transient"
    ]
) }}
...
source_and_target_ids AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN TO_CHAR(t.RFRRL_ID) IS NULL THEN 'I'
