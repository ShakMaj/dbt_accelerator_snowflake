{{ config(
    ...
    pre_hook= [
        "{{ copy_into_macro('bronze_zone_dev.wescom_policy_mgt_ext.wescom_policy_mgt_ext_stage', 'bronze_zone_dev.wescom_policy_mgt_ext.oiv_cov_msg_rel_transient', false, '(?i)TDB2PMED.OIV_COV_MSG_REL/.*.parquet') }}",
         "UPDATE bronze_zone_dev.wescom_policy_mgt_ext.oiv_cov_msg_rel_transient SET AUDIT_CREATED_DATETIME = CURRENT_TIMESTAMP(), AUDIT_CREATED_BY = CURRENT_USER(), HASH_KEY_COLUMNS = SHA2(CONCAT(OIV_XML_ID,' | ',BUS_RSPN_TYP_CD)) WHERE HASH_KEY_COLUMNS IS NULL"
    ]
) }}
