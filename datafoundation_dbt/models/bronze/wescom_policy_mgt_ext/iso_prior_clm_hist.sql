{{ config(
    ...
    pre_hook= [
        "{{ copy_into_macro('bronze_zone_dev.wescom_policy_mgt_ext.wescom_policy_mgt_ext_stage', 'bronze_zone_dev.wescom_policy_mgt_ext.iso_prior_clm_hist_transient', false, '(?i)TDB2PMED.ISO_PRIOR_CLM_HIST/.*.parquet') }}",
         "UPDATE bronze_zone_dev.wescom_policy_mgt_ext.iso_prior_clm_hist_transient SET AUDIT_CREATED_DATETIME = CURRENT_TIMESTAMP(), AUDIT_CREATED_BY = CURRENT_USER(), HASH_KEY_COLUMNS = SHA2(CONCAT(ISO_PRR_CLM_HST_ID)) WHERE HASH_KEY_COLUMNS IS NULL"
    ]
) }}
