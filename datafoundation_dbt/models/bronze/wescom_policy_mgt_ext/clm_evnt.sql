{{ config(
    ...
    pre_hook= [
        "{{ copy_into_macro('bronze_zone_dev.wescom_policy_mgt_ext.wescom_policy_mgt_ext_stage', 'bronze_zone_dev.wescom_policy_mgt_ext.clm_evnt_transient', false, '(?i)TDB2PMED.CLM_EVNT/.*.parquet') }}",
         "UPDATE bronze_zone_dev.wescom_policy_mgt_ext.clm_evnt_transient SET AUDIT_CREATED_DATETIME = CURRENT_TIMESTAMP(), AUDIT_CREATED_BY = CURRENT_USER(), HASH_KEY_COLUMNS = SHA2(CONCAT(CLM_EVNT_ID)) WHERE HASH_KEY_COLUMNS IS NULL"
    ]
) }}
