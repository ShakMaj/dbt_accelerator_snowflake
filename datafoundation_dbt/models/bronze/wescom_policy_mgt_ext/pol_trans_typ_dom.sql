{{ config(
    ...
    pre_hook= [
        "{{ copy_into_macro('bronze_zone_dev.wescom_policy_mgt_ext.wescom_policy_mgt_ext_stage', 'bronze_zone_dev.wescom_policy_mgt_ext.pol_trans_typ_dom_transient', false, '(?i)TDB2PMED.POL_TRANS_TYP_DOM/.*.parquet') }}",
         "UPDATE bronze_zone_dev.wescom_policy_mgt_ext.pol_trans_typ_dom_transient SET AUDIT_CREATED_DATETIME = CURRENT_TIMESTAMP(), AUDIT_CREATED_BY = CURRENT_USER(), HASH_KEY_COLUMNS = SHA2(CONCAT(POL_TRANS_TYP)) WHERE HASH_KEY_COLUMNS IS NULL"
    ]
) }}
